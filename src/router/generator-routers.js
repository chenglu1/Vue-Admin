...
export const generator = (routerMap, parent) => {
  return routerMap.map(item => {
    const { menuname: title, show, hideChildren, hiddenHeaderContent, target, icon } = item.meta || item
    const currentRouter = {
      // 如果路由设置了 path，则作为默认 path，否则 路由地址 动态拼接生成如 /dashboard/workplace
      path: item.page ? `/${item.page.split('#')[1]}` : `${parent && parent.path || ''}/${item.key}`,
      // 路由名称，建议唯一
